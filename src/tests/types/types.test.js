import { types } from "../../redux/types"

describe("Types: ", () => {
  test("types must be the same", () => {
    expect(types).toEqual({
      uiOpenModal: "[ui] Open modal",
      uiCloseModal: "[ui] Close modal",
      setError: "[Error] Error detected",
      unsetError: "[Error] Error clearned",
      fetchPhones: "[Phones] Fetch Phones",
      fetchPhonesSuccess: "[Phones] Fetch Success Phones",
      fetchPhoneByID: "[Phones] Fetch Phone by ID",
      fetchPhoneSuccessById: "[Phones] Fetch Success Phone by ID",
      selectPhone: "[Phones] Select Phone",
      addNewPhone: "[Phones] Add New Phone",
      addNewPhoneSuccess: "[Phones] Add New Phone Success",
      deletePhone: "[Phones] Delete Phone",
      deletePhoneSuccess: "[Phones] Delete Phone Success",
      updatePhone: "[Phones] Update Phone",
      updatePhoneSuccess: "[Phones] Update Phone Success",
      uploadImagePhone: "[Phones] Upload Image Phone",
      uploadImagePhoneSuccess: "[Phones] Upload Image Phone Success",
      authStartLogin: "[auth] Start login",
      authLogin: "[auth] Login",
      authStartRegister: "[auth] Start Register",
      authStartStartTokenRenew: "[auth] Start token renew",
      authLogout: "[auth] Logout",
    })
  })
})
